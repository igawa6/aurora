...
// DXT5/BC3: an 8-byte BC4 alpha block followed by an 8-byte BC1 colour block.
// The colour block is always in four-colour mode here — BC1's "color1 <= color2
// means punch-through alpha" rule does not apply once alpha has its own block.
static ByteBuffer BuildRGBA8FromBC3(uint32_t width, uint32_t height, uint32_t mips, ArrayRef<uint8_t> data) {
  const size_t texelCount = ComputeMippedTexelCount(width, height, mips);
  ByteBuffer buf{sizeof(RGBA8) * texelCount};
      ...
      for (uint32_t xx = 0; xx < w; xx += 4) {
        ...
        if (alphaTable[0] > alphaTable[1]) {
          for (uint32_t i = 0; i < 6; ++i) {
            alphaTable[2 + i] = static_cast<uint8_t>(((6 - i) * alphaTable[0] + (1 + i) * alphaTable[1]) / 7);
          }
        } else {
          for (uint32_t i = 0; i < 4; ++i) {
            alphaTable[2 + i] = static_cast<uint8_t>(((4 - i) * alphaTable[0] + (1 + i) * alphaTable[1]) / 5);
          }
          alphaTable[6] = 0;
          alphaTable[7] = 0xFF;
        }
        // Six bytes of 3-bit indices, little-endian, read as one 48-bit word.
        uint64_t alphaIndices = 0;
        for (uint32_t i = 0; i < 6; ++i) {
          alphaIndices |= static_cast<uint64_t>(src[2 + i]) << (8 * i);
        }

        const uint16_t color1 = *reinterpret_cast<const uint16_t*>(src + 8);
        const uint16_t color2 = *reinterpret_cast<const uint16_t*>(src + 10);
        const uint32_t indices = *reinterpret_cast<const uint32_t*>(src + 12);
        src += 16;
        ...
        colorTable[5] = ExpandTo8<6>(static_cast<uint8_t>((color2 >> 5) & 0x3F));
        colorTable[6] = ExpandTo8<5>(static_cast<uint8_t>(color2 & 0x1F));
        // The S3TC spec's thirds, NOT the S3TCBlend() above it: that one is the
        // GameCube's (3a+5b)/8 approximation of CMPR, correct for the game's own
        // textures and wrong by ~4% for a PC-authored DXT5 file, which is what a
        // replacement always is.
        for (uint32_t c = 0; c < 3; ++c) {
          const uint32_t a = colorTable[c];
          const uint32_t b = colorTable[4 + c];
          colorTable[8 + c] = static_cast<uint8_t>((2 * a + b) / 3);
          colorTable[12 + c] = static_cast<uint8_t>((a + 2 * b) / 3);
        }
        ...
        for (uint32_t y = 0; y < 4; ++y) {
          for (uint32_t x = 0; x < 4; ++x) {
            ...
            const uint32_t index = (indices >> (2 * texel)) & 3;
            const uint32_t alphaIndex = static_cast<uint32_t>((alphaIndices >> (3 * texel)) & 7);
            uint8_t* dstOffs = dst + ((yy + y) * w + (xx + x)) * 4;
            memcpy(dstOffs, &colorTable[static_cast<size_t>(index) * 4], 3);
            dstOffs[3] = alphaTable[alphaIndex];
          }
        }
      }
    }
...
ByteBuffer decompress_bc_to_rgba8(wgpu::TextureFormat format, uint32_t width, uint32_t height, uint32_t mips,
  ...
  ZoneScoped;
  // Both decoders read whole 4x4 blocks; a truncated file would run off the end.
  const auto blocks = [&](uint32_t bytesPerBlock) {
    size_t need = 0;
    uint32_t w = width, h = height;
    for (uint32_t mip = 0; mip < mips; ++mip) {
      need += static_cast<size_t>((w + 3) / 4) * ((h + 3) / 4) * bytesPerBlock;
      w = w > 1 ? w / 2 : 1;
      h = h > 1 ? h / 2 : 1;
    }
    return need;
  ...
  case wgpu::TextureFormat::BC1RGBAUnormSrgb:
    return data.size() >= blocks(8) ? BuildRGBA8FromBC1(width, height, mips, data) : ByteBuffer{};
  case wgpu::TextureFormat::BC2RGBAUnorm:
  case wgpu::TextureFormat::BC2RGBAUnormSrgb:
  case wgpu::TextureFormat::BC3RGBAUnorm:
  case wgpu::TextureFormat::BC3RGBAUnormSrgb:
    // BC2 shares BC3's 16-byte block and colour half; only the alpha encoding
    // differs, and BC2 is vanishingly rare in texture packs, so it is not
    // decoded rather than decoded wrongly.
    if (format == wgpu::TextureFormat::BC2RGBAUnorm || format == wgpu::TextureFormat::BC2RGBAUnormSrgb) {
      return {};
    }
    return data.size() >= blocks(16) ? BuildRGBA8FromBC3(width, height, mips, data) : ByteBuffer{};
  default:
    return {};  // BC4/BC5/BC6H/BC7 — no decoder
  }
}
